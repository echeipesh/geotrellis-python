...
import protos.tileMessages.{Tile => TileMessage,CellType => CellTypeMessage, _}
...
trait TileProtoBuf {
  ...
  private def cellTypeToMessage(ct: CellType): CellTypeMessage = {
    ct match {
      case ByteConstantNoDataCellType =>
        CellTypeMessage(CellTypeMessage.DataType.BYTE, Some(ND(Byte.MinValue)))
      case ByteCellType =>
        CellTypeMessage(CellTypeMessage.DataType.BYTE, None)
      case ByteUserDefinedNoDataCellType(v) =>
        CellTypeMessage(CellTypeMessage.DataType.BYTE, Some(ND(v)))
    }
  }

  private def messageToCellType(ctm: CellTypeMessage): CellType = {
    ctm match {
      case CellTypeMessage(CellTypeMessage.DataType.BYTE, nd) =>
        ByteCells.withNoData(nd.map(_.value.toByte))
    }
  }

  private def intVectorToTile(v: Vector[Int], cellType: CellType): Tile = {
    val arr = Array.ofDim[Int](v.length)
    ???
  }

  private def floatVectorToTile(v: Vector[Float], cellType: CellType): Tile = {
    val arr = Array.ofDim[Float](v.length)
    ???
  }

  implicit def tileProtoBufCodec = new ProtoBufCodec[Tile, TileMessage] {
    def encode(tile: Tile): TileMessage = {
      TileMessage(
        cols = tile.cols,
        rows = tile.rows,
        cellType = Some(cellTypeToMessage(tile.cellType)),
        sint32Cells = tile.toArray(),
        uint32Cells = null
      )
    }

    def decode(message: TileMessage): Tile = {
      val ct = messageToCellType(message.cellType.get)

      ct match {
        case ByteCells => intVectorToTile(message.sint32Cells, ct)
        case FloatCells => floatVectorToTile(message.floatCells, ct)
      }
    }
  }
